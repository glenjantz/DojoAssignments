...
//        boss.position = CGPoint(x: 265, y: 100)
        boss.position = CGPoint(x: self.size.width/2 - 100, y: self.size.height/2 - 76)
        self.addChild(boss)
        ...
        let initialPlayerPosition = CGPoint(x:0, y:self.size.height - 240)
//        let initialPlayerPosition = CGPoint(x:0, y:self.size.height - 60)
        ...
        backgrounds[0].backgroundSize = CGSize(width: self.size.width, height: self.size.height)
//        backgrounds[0].backgroundSize = CGSize(width: 1024, height: 768)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if (contact.bodyA.categoryBitMask == PhysicsCategory.enemy.rawValue && contact.bodyB.categoryBitMask == PhysicsCategory.enemy.rawValue) || (contact.bodyA.categoryBitMask == PhysicsCategory.enemy.rawValue && contact.bodyB.categoryBitMask == PhysicsCategory.enemy.rawValue) || (contact.bodyA.categoryBitMask == PhysicsCategory.enemy.rawValue && contact.bodyB.categoryBitMask == PhysicsCategory.ground.rawValue) || (contact.bodyA.categoryBitMask == PhysicsCategory.ground.rawValue && contact.bodyB.categoryBitMask == PhysicsCategory.enemy.rawValue) {
            ...
            boss.takeDamage()
            let bee = Bee()
            bee.position = CGPoint(x: boss.position.x - 200, y: boss.position.y)
            self.addChild(bee)
